...
export async function findIvrCallBackUrl(where: any, callback: any) {
  try {
    let Query = "SELECT  sme_id ,crm_partner, status ,insert_datetime ,update_datetime,recording_url,recording_url_provider, outgoing_url,outgoing_url_provider,incoming_url ,incoming_url_provider,outgoing_auth_token,incoming_auth_token,recording_auth_token,call_popup_url,call_popup_auth_token,call_popup_provider from sme_crm_details  WHERE sme_id = :smeId limit 1";
    let executeQuery = await sequelize.query<any>(Query, {
      raw: true,
      type: QueryTypes.SELECT,
      replacements: { smeId: where["smeId"] },
