...
export async function FindIvrFlow(where: any, callback: any) {
  try {
    let Query =
      "Select id, sme_id as smeId, cat_id as catId, flow_id as flowId, flow_name as flowName, cat_desc as catDescription, parent_cat_id as parentId, children as child, dtmf, media_file_status as mediaFileStatus, media_file as mediaFilePath, service_type as servicrType, title as catTitle, event_type as eventType, type, media_id as media, queue_id as queue, retry, redirect_to as redirectTo, dtmf_wait_time as waitTime from mpbx_category_master where sme_id = :id AND (flow_id = :flowId OR flow_id IS NULL OR flow_id = '') ORDER BY id ASC";
    let executeQuery = await sequelize.query<any>(Query, {
      raw: true,
      type: QueryTypes.SELECT,
...
export async function insertUpdateIvrFlow(payload: any, callback: any) {
  try {
    let Query =
      "INSERT INTO mpbx_category_master (sme_id, cat_id, cat_desc, parent_cat_id,children, dtmf, media_file_status, media_file, title, event_type, type, date_time, flow_id, flow_name, queue_id, retry, redirect_to, media_id, dtmf_wait_time) values (:sme_id, :catId, :catDescription , :parentId, :child, :dtmf, :mediaFileStatus, :mediaFilePath, :catTitle, :eventType, :type, :insertDateTime, :flowId, :flowName, :queueId, :retry, :redirectTo, :mediaId, :dtmfWaitTime)";
    ...
    let executeQuery = await sequelize.query(Query, {
      ...
      replacements: {
        ...
        dtmfWaitTime: payload["dtmfWaitTime"],
      },
    });
    callback(null, executeQuery);
  } catch (error: any) {
...
export async function FindagentInsight(where: fetchInsightRequest, callback: any) {
  try {
    ...
    if (where["agentId"] != 0) {
      agent_id = "and ad.agent_id=" + where["agentId"];
    }

      let Query =
      "SELECT SUM(acd.total_in_calls) AS totalInCalls, SUM(acd.avg_call_duration) AS avgCallDuration, SUM(acd.in_failed_calls) AS inFailedCalls, SUM(acd.in_success_calls) AS inSuccessCalls, SUM(acd.total_out_calls) AS totalOutCalls, SUM(acd.total_calls) AS totalCalls, SUM(acd.total_call_duration) AS totalCallDuration, SUM(acd.out_success_calls) AS outSuccessCalls, (SUM(acd.out_success_calls) * 100) / (SUM(acd.total_out_calls)) AS outSuccess, SUM(acd.out_failed_calls) AS outFailedCalls, SUM(acd.office_hours) AS officeHours, SUM(acd.no_answer) AS noAnswer, SUM(acd.lunch_hours) AS lunchHours, (SUM(acd.in_success_calls) * 100)/(SUM(acd.total_in_calls)) AS inSuccess, SUM(acd.total_connected_duration) AS totalConnectedDuration, SUM(acd.avg_connected_duration) AS avgConnectedDuration, SUM(acd.total_ringing_duration) AS totalRingingDuration, SUM(acd.avg_ringing_duration) AS avgRingingDuration FROM agent_details ad LEFT JOIN agent_calling_details acd ON acd.agent_id = ad.agent_id AND acd.insert_date BETWEEN :startDate AND :endDate WHERE ad.sme_id = :sme_id " +
      agent_id +
      "";
    let executeQuery = await sequelize.query<any>(Query, {
      raw: true,
...
export async function checkCanCreateAgent(where: fetchRequest, callback: any) {
  try {
    let Query = "SELECT sp.allowed_agents, COUNT(ad.agent_id) AS total_agents FROM sme_profile sp LEFT JOIN agent_details ad ON sp.id = ad.sme_id WHERE sp.`status` != -9 AND sp.id = :id";
    let executeQuery = await sequelize.query<any>(Query, {
      raw: true,
      type: QueryTypes.SELECT,
      replacements: { id: where["id"] },
...
export async function FindBlacklistNumbers(where: any, callback: any) {
  try {
    let limit_field = "";
    let initialRecord;
...
export async function getAgentsInsightDayWiseData(where: any, callback: any) {
  try {
    var agent_id_field = "";
    if (where["agentId"] && where["agentId"] != "") {
      agent_id_field = " and ad.agent_id = " + where["agentId"];
    }

    var Query = "SELECT SUM(acd.in_failed_calls) AS in_failed_calls, SUM(acd.in_success_calls) AS in_success_calls, SUM(acd.total_in_calls) AS total_in_calls, SUM(acd.out_failed_calls) AS out_failed_calls, SUM(acd.out_success_calls) AS out_success_calls, SUM(acd.total_out_calls) AS total_out_calls, SUM(acd.total_calls) AS total_calls, SUM(acd.avg_call_duration) AS avg_call_duration, SUM(acd.total_call_duration) AS total_call_duration, SUM(acd.office_hours) AS office_hours, SUM(acd.lunch_hours) AS lunch_hours, SUM(acd.no_answer) AS no_answer, acd.insert_date, DATE_FORMAT(acd.insert_date, '%Y-%m-%d') AS insert_date_format, SUM(acd.connected_duration) AS connected_duration, SUM(acd.ringing_duration) AS ringing_duration FROM agent_details ad LEFT JOIN agent_calling_details acd ON acd.agent_id = ad.agent_id AND acd.insert_date BETWEEN :startDate AND :endDate WHERE ad.sme_id = :id " +
    agent_id_field +
    " GROUP BY DATE_FORMAT(acd.insert_date, '%Y-%m-%d') ORDER BY acd.insert_date ASC";  
...
export async function getIvrFlowlongcodeIdData(where: any, callback: any) {
  try {
    let Query = "SELECT lsm.longcode_id as longcodeId, lsm.longcode_id as id, l.longcode FROM longcodes_sme_mapping lsm LEFT JOIN longcodes l ON l.id = lsm.longcode_id WHERE lsm.sme_id = :sme_id AND lsm.call_flow_id = :flowId ";
    let executeQuery = await sequelize.query(Query, {
      raw: true,
      type: QueryTypes.SELECT,
...
export async function deleteCampaignBaseData(where: any, callback: any) {
  try {
    let Query = "delete from upload_dialer_numbers where campaign_id = :campaignId AND sme_id = :smeId";
    let executeQuery = await sequelize.query(Query, {
      raw: true,
      type: QueryTypes.UPDATE,
      replacements: {
...
export async function getOutgoingCampaigAgentWiseData(where: any, callback: any) {
  try {
    let Query = "SELECT COUNT(*) as assigned, udn.customer_number, ad.agent_name, oc.campaign_name, COUNT(IF(udn.call_status = 1, 1, NULL)) AS answered, COUNT(IF(udn.call_status = 2, 1, NULL)) AS failed, COUNT(IF(udn.call_status = 0, 1, NULL)) AS pending FROM upload_dialer_numbers udn LEFT JOIN agent_details ad ON ad.agent_id = udn.assigned_agent_id LEFT JOIN outgoing_campaign oc ON udn.sme_id = oc.sme_id AND udn.campaign_id = oc.id WHERE udn.sme_id = :smeId AND udn.campaign_id = :campaignId GROUP BY udn.assigned_agent_id";
    let executeQuery = await sequelize.query(Query, {
      raw: true,
      type: QueryTypes.SELECT,
      replacements: {
...
export async function getAgentCampaignData(where: any, callback: any) {
  try {
    ...
    let Query = "SELECT udn.id, udn.campaign_id, udn.customer_number, udn.customer_number as customerNumber, udn.other, udn.call_mode, is_picked, DATE_FORMAT(oc.start_date_time, '%Y-%m-%d %H:%i:%s') as campaign_start_date, udn.call_status, udn.session_id, DATE_FORMAT(oc.end_date_time, '%Y-%m-%d %H:%i:%s') as campaign_end_date, oc.campaign_name, ab.customer_name FROM upload_dialer_numbers udn LEFT JOIN outgoing_campaign oc ON oc.sme_id = udn.sme_id AND oc.id = udn.campaign_id LEFT JOIN address_book ab ON ab.customer_number_primary = udn.customer_number AND ab.sme_id = udn.sme_id WHERE udn.sme_id = :smeId AND udn.assigned_agent_id = :agentId AND (oc.start_date_time < :currentDate AND oc.end_date_time > :currentDate) ORDER BY udn.is_picked ASC "+limit_field+" ";
    let executeQuery = await sequelize.query<any>(Query, {
      raw: true,
      type: QueryTypes.SELECT,
...
export async function InserFacebookleadSetting(where: any, callback: any) {
  try {
    let Query = "INSERT INTO facebook_lead_setting (third_party_code, status, client_id, secret_key, access_token,sme_id,insert_date_time,token_expired_date) values ('facebook',:status, :clientId, :secretKey, :token, :id,:insertDateTime,:tokenExpired)";
...
export async function insertUniqueCustomerCampaignNew(payload: any, reqData: any, callback: any) {
  try {
    let Query =
      "INSERT INTO upload_dialer_numbers (sme_id, customer_number, lead_status, city_id, product_id, product_price, assigned_agent_id, insert_date_time, update_date_time, call_type, assigned_to, campaign_id, source_id, other, call_status, session_id, call_mode) values (:smeId, :customerNumber, :leadStatus, :cityId, :productId, :productPrice, :assignedAgentId, :insertDateTime, :updateDateTime, :callType, :agentId, :campaignId, :sourceId, :other, :callStatus, :sessionId, :callMode)";
...
export async function getOutgoingCampaignBetaData(where: any, callback: any) {
  try {
    ...
    let Query = 'SELECT oc.id, oc.campaign_name, oc.is_report_generated, oc.campaign_description, oc.campaign_type, oc.status,   DATE_FORMAT(oc.date_time, "%Y-%m-%d %H:%i:%s") AS date_time, DATE_FORMAT(oc.start_date_time, "%Y-%m-%d %H:%i:%s") AS start_date_time,    DATE_FORMAT(oc.end_date_time, "%Y-%m-%d %H:%i:%s") AS end_date_time, oc.base_count, oc.invalid_count, oc.filtered_count, oc.duplicate_count,  oc.already_assigned, oc.agent_wise_report_path, oc.answered_report_path, oc.failed_report_path, oc.pending_report_path, oc.campaign_report_path,  oc.start_stop_status, oc.failed_count, oc.success_count, oc.pending_count, oc.assigned_agents, oc.executed  FROM outgoing_campaign oc  WHERE oc.sme_id = :smeId AND oc.status != "-9" AND (oc.campaign_type = "Autodialer New" OR oc.campaign_type = "Preview Dialer" OR oc.campaign_type = "Preview Dialer New")' +
    campaign_name_field +
    ''+
    campaign_description_field +
    ''+
...
export async function getMergeCallsData(where: any, callback: any) {
  try {
    let customer_number_field = "";
    let customer_name_field = "";
    let agent_number_field = "";
    ...
    if (where["agentNumber_op"] == "1" && where["teamLead"] == "0") {
      //Equals to
    ...
    if (where["callQueue_op"] == "1") {
      //Equals to
      callQueue_field = "and cdr.queue_id in('" + where["callQueue"] + "')";
    }
...
export async function getAgentMergeCallsData(where: any, callback: any) {
  try {
    let customer_number_field = "";
    let customer_name_field = "";
    let agent_number_field = "";
    ...
    var Query =
        'SELECT ard.agent_id AS patched_agent_id, ad.agent_mobile, ad.agent_email, ad.recording_type, ad.agent_name AS agent_name,cdr.answer AS answer,cdr.call_direction AS call_direction, cdr.call_direction_status AS call_direction_status, cdr.session_id AS session_id,cdr.call_recorded_file AS call_recorded_file, cdr.call_recording_status AS call_recording_status, ard.status AS call_status,ad.agent_mobile AS called_number, cdr.customer_number, cdr.customer_number as customerNumber, cdr.cdr_mode AS cdr_mode,cdr.channel_no AS channel_no,ab.customer_name AS customer_name, cdr.disconnected_by AS disconnected_by, cdr.duration AS duration, DATE_FORMAT(ard.start_date, "%Y-%m-%d %H:%i:%s") AS start_date_time, DATE_FORMAT(cdr.end_date_time, "%Y-%m-%d %H:%i:%s") AS end_date_time, cdr.hlr AS hlr, cdr.connected_duration, cdr.ringing_duration, DATE_FORMAT(ard.insert_date, "%Y-%m-%d %H:%i:%s") AS insert_date_time, cdr.longcode AS longcode, cdr.call_type, cdr.master_shortcode AS master_shortcode, cdr.merge_status AS merge_status, rec.merged_file, cdr.server_ip_address AS server_ip_address, cdr.shortcode_mapping AS shortcode_mapping,cdr.sme_identifier AS sme_identifier, ard.start_date AS startDate_TimeS,cdr.voicemail_recording_file AS voicemail_recording_file, cdr.voicemail_recording_status AS voicemail_recording_status,cdr.address_book_id AS address_book_id, cdr.dtmf, cr.remarks, cdr.call_status AS iicc_call_status, cdr.call_flow_name AS flow_name, cdr.queue_name, cdr.queue_id, bwl.blacklist_status FROM agent_report_details AS ard LEFT JOIN calling_cdr AS cdr ON cdr.session_id = ard.session_id AND cdr.customer_status = 0 LEFT JOIN mpbx_call_recording rec ON rec.call_id = cdr.call_recorded_file LEFT JOIN address_book ab ON ab.customer_number_primary = cdr.customer_number AND ab.sme_id = cdr.sme_id LEFT JOIN customer_remarks cr ON cr.session_id = cdr.session_id LEFT JOIN black_white_list bwl ON bwl.customer_number = cdr.customer_number AND bwl.sme_id = cdr.sme_id AND bwl.blacklist_status != -9 LEFT JOIN agent_details AS ad ON ard.agent_id = ad.agent_id WHERE (ard.insert_date BETWEEN :startDate AND :endDate) AND cdr.sme_id = :smeId AND ard.agent_id= :agentId AND cdr.customer_status = 0 AND cdr.call_type != "3" AND (bwl.blacklist_status is NULL OR bwl.blacklist_status = 0)' + 
        " " +
        duration_field +
        " " +
        customer_number_field +
    ...
    let executeQuery = await sequelize.query<any>(Query, {
      raw: true,
      type: QueryTypes.SELECT,
      replacements: { smeId: where["smeId"], startDate: where["startDate"], endDate: where["endDate"], agentId: where["agentId"] },
    });
    callback(null, executeQuery);
  } catch (error: any) {
    logger.error(error);
...
export async function getQueueData(where: any, callback: any) {
  try {
    ...
    let Query = "SELECT sq.id, sq.sme_id, sq.name, sq.description, sq.`status`, DATE_FORMAT(sq.insert_date_time, '%Y-%m-%d %H:%i:%s') as insert_date_time FROM sme_queue sq WHERE sq.sme_id = :smeId AND sq.status = 1 ORDER BY sq.insert_date_time DESC "+limit_field+" ";
    let executeQuery = await sequelize.query(Query, {
      raw: true,
      type: QueryTypes.SELECT,
      replacements: {
...
export async function deleteQueueMappingData(payload: any, callback: any) {
  try {
    let Query = "DELETE from sme_queue_mapping  WHERE sme_id = :smeId AND queue_id = :queueId";
    let executeQuery = await sequelize.query(Query, {
      raw: true,
      type: QueryTypes.DELETE,
      replacements: { smeId: payload["smeId"], queueId: payload["queueId"] },
    });
    callback(null, executeQuery);
  } catch (error: any) {
    logger.error(error);
...
export async function getQueueAssignedAgentsData(where: any, callback: any) {
  try {
    let Query = "SELECT sqm.sme_id, sqm.agent_id, ad.agent_name FROM sme_queue_mapping sqm LEFT JOIN agent_details ad ON ad.agent_id = sqm.agent_id WHERE sqm.sme_id = :smeId AND sqm.queue_id = :queueId AND ad.agent_name != 'null'";
    let executeQuery = await sequelize.query(Query, {
      raw: true,
      type: QueryTypes.SELECT,
      replacements: {
...
export async function getAgentQueueData(where: any, callback: any) {
  try {
...
export async function getMediaData(where: any, callback: any) {
  try {
    let limit_field = "";
    let initialRecord;
    ...
    let Query = "SELECT * FROM media_details md WHERE md.sme_id = :smeId AND md.status = 1  ORDER BY md.insert_date_time DESC "+limit_field+" ";
    let executeQuery = await sequelize.query(Query, {
      raw: true,
      type: QueryTypes.SELECT,
      replacements: {
...
export async function addMediaData(payload: any, callback: any) {
  try {
    let Query =
      "INSERT INTO media_details (sme_id, name, media_type, language, text_to_speech, file_path, directory_path, file_name, file_original_name, status, insert_date_time, update_date_time) values (:smeId, :name, :mediaType, :language, :textToSpeech, :filePath, :directoryPath, :fileName, :fileOriginalName, :status, :insertDateTime, :updateDateTime)";
    ...
    let executeQuery = await sequelize.query(Query, {
      ...
      replacements: {
        ...
        directoryPath: payload["directoryPath"],
        fileName: payload["fileName"],
        fileOriginalName: payload["fileOriginalName"],
        status: payload["status"],
        insertDateTime: payload["insertDateTime"],
        updateDateTime: payload["updateDateTime"],
      },
...
export async function updateMediaData(where: any, callback: any) {
  try {
    let Query = "update media_details set status = :status where sme_id = :smeId AND id = :mediaId";
    let executeQuery = await sequelize.query(Query, {
      raw: true,
      type: QueryTypes.UPDATE,
      replacements: {
...
export async function updateIsPickedData(where: any, callback: any) {
  try {
    let Query = "update upload_dialer_numbers set is_picked = 1 where sme_id = :smeId AND id = :id";
    let executeQuery = await sequelize.query(Query, {
      raw: true,
      type: QueryTypes.UPDATE,
      replacements: {
...
export async function resetSmeLongcodeFlowMapping(where: any, callback: any) {
  try {
    let Query = "update longcodes_sme_mapping set call_flow_id = 0 where sme_id = :sme_id AND call_flow_id = :flowId";
    let executeQuery = await sequelize.query(Query, {
      raw: true,
      type: QueryTypes.UPDATE,
      replacements: {
...
export async function getLiveAprData(where: any, callback: any) {
  try {
    let limit_field = "";
    let initialRecord;
    initialRecord = where["initialRecord"] - 1;
    ...
    let Query = "SELECT ad.agent_name, ad.agent_mobile, ad.status, ad.agent_id, COALESCE(SUM(acd.total_call_duration), 0) AS total_call_duration, COALESCE(SUM(acd.avg_call_duration), 0) AS avg_call_duration, COALESCE(SUM(acd.total_connected_duration), 0) AS total_connected_duration, COALESCE(SUM(acd.avg_connected_duration), 0) AS avg_connected_duration, COALESCE(SUM(acd.total_ringing_duration), 0) AS total_ringing_duration, COALESCE(SUM(acd.avg_ringing_duration), 0) AS avg_ringing_duration, COALESCE(SUM(acd.total_calls), 0) AS total_calls, COALESCE(SUM(acd.total_in_calls), 0) AS total_incoming, COALESCE(SUM(acd.total_out_calls), 0) AS total_outgoing, COALESCE(SUM(acd.in_success_calls), 0) AS incoming_success, COALESCE(SUM(acd.in_failed_calls), 0) AS incoming_failed, COALESCE(SUM(acd.out_success_calls), 0) AS outgoing_success, COALESCE(SUM(acd.out_failed_calls), 0) AS outgoing_failed, COALESCE(SUM(acd.office_hours), 0) AS office_hours, COALESCE(SUM(acd.no_answer), 0) AS no_answer, COALESCE(SUM(acd.lunch_hours), 0) AS lunch_hours FROM agent_details ad LEFT JOIN agent_calling_details acd ON acd.agent_id = ad.agent_id AND (acd.insert_date BETWEEN :startDate and :endDate) WHERE ad.sme_id = :smeId  GROUP BY ad.agent_id";
...
export async function getWhatsappData(where: any, callback: any) {
  try {
    let Query = "select wd.id, wd.name, wd.url, wd.method_type, wd.token_type, wd.username, wd.password, wd.token, wd.custom_header_key, wd.custom_header_value, wd.user_fields, wd.json_field, wd.fixed_field, wd.custom_field, DATE_FORMAT(wd.insert_date_time, '%Y-%m-%d %H:%i:%s') as insert_date_time from whatsapp_template wd where sme_id = :sme_id and status != -9";
    let executeQuery = await sequelize.query(Query, {
      raw: true,
      type: QueryTypes.SELECT,
      replacements: {
...
export async function addWhatsappTemplateData(payload: any, callback: any) {
  try {
    let Query =
      "INSERT INTO whatsapp_template (sme_id, name, url, token_type, method_type, username, password, token, user_fields, custom_header_key, custom_header_value, json_field, fixed_field, custom_field, status, insert_date_time, update_date_time) values (:sme_id, :name, :url, :token_type, :method_type, :username, :password, :token, :user_fields, :custom_header_key, :custom_header_value, :json_field, :fixed_field, :custom_field, :status, :insert_date_time, :update_date_time)";
    ...
    let executeQuery = await sequelize.query(Query, {
      ...
      replacements: {
        ...
        custom_header_key: payload["custom_header_key"],
        custom_header_value: payload["custom_header_value"],
        json_field: payload["json_field"],
        fixed_field: payload["fixed_field"],
...
export async function deleteWhatsappTemplateData(where: any, callback: any) {
  try {
    let Query = "update whatsapp_template set status = :status where sme_id = :sme_id AND id = :id";
    let executeQuery = await sequelize.query(Query, {
      raw: true,
      type: QueryTypes.UPDATE,
      replacements: {
...
export async function addWhatsAppMappingData(payload: any, callback: any) {
  try {
    let Query =
      "INSERT INTO endcall_whatsapp_mapping (sme_id, whatsapp_enum_id, template_id, ivr_flow_id, insert_date_time, update_date_time) values (:sme_id, :whatsapp_enum_id, :template_id, :ivr_flow_id, :insert_date_time, :update_date_time)";
    ...
    let executeQuery = await sequelize.query(Query, {
      ...
      replacements: {
        sme_id: payload["sme_id"],
        whatsapp_enum_id: payload["whatsapp_enum_id"],
        template_id: payload["template_id"],
        ivr_flow_id: payload["ivr_flow_id"],
        insert_date_time: payload["insert_date_time"],
        update_date_time: payload["update_date_time"],
      },
    });
...
export async function getWhatsappMappingData
(where: any, callback: any) {
  try {
    let flow_id_condition = "";
    if(where["ivr_flow_id"] && where["ivr_flow_id"] > 0){
      flow_id_condition = " and ivr_flow_id = "+where["ivr_flow_id"]+"";
    }
    let Query = "select * from endcall_whatsapp_mapping wd where sme_id = :sme_id "+flow_id_condition;
    let executeQuery = await sequelize.query(Query, {
      raw: true,
      type: QueryTypes.SELECT,
      replacements: {
...
export async function deleteWhatsAppMappingData(where: any, callback: any) {
  try {
    let Query = "DELETE from  endcall_whatsapp_mapping where sme_id = :sme_id and ivr_flow_id = :ivr_flow_id";
    let executeQuery = await sequelize.query(Query, {
      raw: true,
      type: QueryTypes.DELETE,
      replacements: {
        sme_id: where["sme_id"],
        ivr_flow_id: where["ivr_flow_id"],
      },
    });
    callback(null, executeQuery);
  } catch (error: any) {
...
export async function getWhatsappEnumData(where: any, callback: any) {
  try {
    let Query = "select * from whatsapp_enum where status = 1";
    let executeQuery = await sequelize.query(Query, {
      raw: true,
      type: QueryTypes.SELECT,
      replacements: {},
...
export async function getWhatsappMappingIvrCountData(where: any, callback: any) {
  try {
    let Query = "SELECT COUNT(DISTINCT ivr_flow_id) AS whatsapp_ivr_count from endcall_whatsapp_mapping ewm where sme_id = :sme_id ";
    let executeQuery = await sequelize.query(Query, {
      raw: true,
      type: QueryTypes.SELECT,
      replacements: {
...
export async function editWhatsappTemplateData(where: any, callback: any) {
  try {
    let Query = "update whatsapp_template set name = :name, url = :url, token_type = :token_type, method_type = :method_type, username = :username, password = :password, token = :token, custom_header_key = :custom_header_key, custom_header_value = :custom_header_value, json_field = :json_field, fixed_field = :fixed_field, custom_field = :custom_field, user_fields = :user_fields, update_date_time = :update_date_time where sme_id = :sme_id AND id = :template_id ";
    let executeQuery = await sequelize.query(Query, {
      raw: true,
      type: QueryTypes.UPDATE,
      replacements: {
        ...
        name: where["name"],
        url: where["url"],
        token_type: where["token_type"],
        method_type: where["method_type"],
        username: where["username"],
        password: where["password"],
        token: where["token"],
        custom_header_key: where["custom_header_key"],
        custom_header_value: where["custom_header_value"],
        json_field: where["json_field"],
        fixed_field: where["fixed_field"],
        custom_field: where["custom_field"],
        user_fields: where["user_fields"],
        update_date_time: where["update_date_time"],
        template_id: where["template_id"]        
      },
    });
    callback(null, executeQuery);
...
export async function deleteWhatsappMapping(where: any, callback: any) {
  try {
    let Query = "DELETE from  endcall_whatsapp_mapping where sme_id = :sme_id and template_id = :id";
    let executeQuery = await sequelize.query(Query, {
      raw: true,
      type: QueryTypes.DELETE,
      replacements: {
