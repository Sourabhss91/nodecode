...
export async function totalRecordList(where: any, callback: any) {
  try {
    ...
    if (table_name == "outbond_ivr_cdr") {
      ...
      Query =
        "SELECT COUNT(DISTINCT ard.session_id) AS total_records FROM agent_report_details AS ard LEFT JOIN agent_details AS ad ON ard.agent_id = ad.agent_id LEFT JOIN incoming_ivr_call_cdr AS ivr ON ivr.session_id = ard.session_id AND ivr.customer_status = 0 LEFT JOIN address_book ab ON SUBSTRING(TRIM(ab.customer_number_primary), -10) = SUBSTRING(ivr.calling_number, -10) LEFT JOIN mpbx_call_recording rec ON rec.call_id = ivr.call_recorded_file LEFT JOIN customer_remarks cr ON cr.session_id = ivr.session_id WHERE (ard.insert_date BETWEEN :startDate AND :endDate) AND ivr.sme_id= :smeId AND ard.agent_id= :id AND ard.call_info = 'incoming' " +
        duration_field +
        " " +
        called_number_field +
        " " +
