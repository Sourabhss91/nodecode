...
export async function UserSignin(where: signinRequest, callback: any) {
  try {
    let Query = "SELECT U.username, UR.ROLE, UR.user_role_id FROM users U, user_roles UR WHERE UR.username=U.username AND U.username = :username AND U.password= :password LIMIT 1";
    let executeQuery = await sequelize.query<any>(Query, {
      raw: true,
      type: QueryTypes.SELECT,
...
export async function getQueueData(where: any, callback: any) {
  try {
    ...
    let Query = "SELECT sq.id, sq.sme_id, sq.name, sq.description, sq.`status`, DATE_FORMAT(sq.insert_date_time, '%Y-%m-%d %H:%i:%s') as insert_date_time FROM sme_queue sq WHERE sq.sme_id = :smeId  ORDER BY sq.insert_date_time DESC "+limit_field+" ";
    let executeQuery = await sequelize.query(Query, {
      raw: true,
      type: QueryTypes.SELECT,
      replacements: {
...
export async function getMediaData(where: any, callback: any) {
  try {
    ...
    let Query = "SELECT * FROM media_details md WHERE md.sme_id = :smeId   ORDER BY md.insert_date_time DESC "+limit_field+" ";
    let executeQuery = await sequelize.query(Query, {
      raw: true,
      type: QueryTypes.SELECT,
      replacements: {
