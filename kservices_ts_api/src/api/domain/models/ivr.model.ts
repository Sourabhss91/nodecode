...
export async function FindSmeIvrFlow(where: any, callback: any) {
  try {
    let Query =
      "SELECT mcm.id, mcm.sme_id, mcm.flow_id, mcm.flow_name, mcm.cat_id, mcm.cat_desc, mcm.parent_cat_id, mcm.children, mcm.dtmf, mcm.media_file_status, mcm.media_file, mcm.service_type,mcm.title, mcm.event_type, mcm.type , mcm.date_time FROM mpbx_category_master AS mcm INNER  JOIN longcodes AS lc ON  SUBSTRING(lc.longcode,  -10) = SUBSTRING(:longcode,  -10) INNER  JOIN longcodes_sme_mapping AS lsm ON lsm.sme_id =mcm.sme_id and lc.id =lsm.longcode_id AND lsm.call_flow_id = mcm.flow_id  WHERE mcm.sme_id =:sme_id ";
...
export async function getAllAgentsStatus(where: any, callback: any) {
  try {
    ...
    let executeQuery = await sequelize_reader.query(Query, {
      raw: true,
      type: QueryTypes.SELECT,
      replacements: { sme_id: where["sme_id"], g_group: where["g_group"] },
    });
    callback(null, executeQuery);
  } catch (error: any) {
  ...
  try {
    ...
    let executeQuery = await sequelize.query(Query, {
      ...
      replacements: {
        ...
        g_agentgroup: where["g_agentgroup"],
  ...
  try {
    ...
    if (where["g_agent_ignore_list"].length > 0) {
      ...
      if(where["g_assigned_vn_agent_id"]){
        ...
        //SELECT ad.*, agd.group_name,agd.group_id,agd.group_status,sm.recording , (  select (case when count(1) > '0' then 'available' else 'off_hours' END ) as status from              agent_details_timing adt where adt.days_week = UPPER(DATE_FORMAT('2023-04-10 17:17:57', '%a'))       and adt.agent_id = ad.agent_id AND (CASE WHEN adt.in_time > adt.out_time THEN ( DATE_FORMAT(adt.in_time, '%T') < DATE_FORMAT('2023-04-10 17:17:57', '%T') OR DATE_FORMAT(adt.out_time, '%T') > DATE_FORMAT('2023-04-10 17:17:57', '%T') ) ELSE DATE_FORMAT(adt.in_time, '%T') < DATE_FORMAT('2023-04-10 17:17:57', '%T') END) AND (CASE WHEN adt.in_time > adt.out_time THEN (DATE_FORMAT(adt.out_time, '%T') < DATE_FORMAT('2023-04-10 17:17:57', '%T') OR DATE_FORMAT(adt.out_time, '%T') > DATE_FORMAT('2023-04-10 17:17:57', '%T')) ELSE DATE_FORMAT(adt.out_time, '%T') > DATE_FORMAT('2023-04-10 17:17:57', '%T') END)) as agent_off_hours_status FROM agent_details ad left JOIN agent_details_timing adt ON ad.agent_id = adt.agent_id LEFT JOIN agent_group_mapping AS ag ON ad.agent_id = ag.agent_id LEFT JOIN agent_group_detail AS agd ON agd.group_id = ag.group_id LEFT JOIN sme_profile AS sm ON ad.sme_id = sm.id  WHERE ad.sme_id  =20002004 and ad.agent_id ='205'  and ad.status !=-9 Limit 1
        Query =
...
export async function getRandomAgent(where: any, callback: any) {
  try {
    ...
    if (where["g_agent_ignore_list"].length > 0) {
      //Query = "select ad.agent_id, ad.agent_name, ad.agent_mobile, ad.status, ad.agent_position, ad.agent_email, ad.sticky_agent , ad.agent_masking, agd.group_name from agent_details ad left JOIN agent_details_timing adt ON ad.agent_id =adt.agent_id left JOIN agent_group_mapping agm ON ad.agent_id =agm.agent_id left JOIN agent_group_detail agd ON agm.group_id =agd.group_id WHERE ad.sme_id =:sme_id and ad.status ='1' and adt.days_week=UPPER(DATE_FORMAT(:currentDate,'%a')) AND DATE_FORMAT(adt.in_time,'%T') < DATE_FORMAT(:currentDate,'%T') AND DATE_FORMAT(adt.out_time,'%T') > DATE_FORMAT(:currentDate,'%T')  AND agd.group_name = (select agd2.group_name from agent_group_detail agd2 where agd2.group_id =:g_group) AND ad.agent_id NOT IN (:g_agent_ignore_list) order by rand() limit 1";
      Query =
    ...
    let executeQuery = await sequelize_reader.query(Query, {
      raw: true,
      type: QueryTypes.SELECT,
      replacements: { sme_id: where["sme_id"], g_group: where["g_group"], currentDate: where["currentDate"], g_agent_ignore_list: where["g_agent_ignore_list"] },
    });
    callback(null, executeQuery);
  } catch (error: any) {
...
export async function getSerialAgent(where: any, callback: any) {
  try {
    ...
    //if(where['g_agent_ignore_list']) {
    if (where["g_agent_ignore_list"].length > 0) {
      //Query = "select ad.agent_id, ad.agent_name, ad.agent_mobile, ad.status, ad.agent_position, ad.agent_email, ad.sticky_agent, agd.group_name from agent_details ad left JOIN agent_details_timing adt ON ad.agent_id =adt.agent_id  left JOIN agent_group_mapping agm ON ad.agent_id =agm.agent_id left JOIN agent_group_detail agd ON agm.group_id =agd.group_id WHERE ad.sme_id =:sme_id and ad.status ='1' and adt.days_week=UPPER(DATE_FORMAT(:currentDate,'%a')) AND DATE_FORMAT(adt.in_time,'%T') < DATE_FORMAT(:currentDate,'%T') AND DATE_FORMAT(adt.out_time,'%T') > DATE_FORMAT(:currentDate,'%T') AND agd.group_name = (select agd2.group_name from agent_group_detail agd2 where agd2.group_id =:g_group) AND ad.agent_id NOT IN (:g_agent_ignore_list) order by agent_position limit 1";
      Query =
...
export async function getEqualAgent(where: any, callback: any) {
  try {
    ...
    //if(where['g_agent_ignore_list']) {
    if (where["g_agent_ignore_list"].length > 0) {
      //Query = "select ad.agent_id,ad.agent_name,ad.agent_mobile,ad.status,ad.agent_position,ad.agent_email,  ad.sticky_agent, ad.agent_masking, agd.group_name,(select count(*) from agent_report_details ard where ard.sme_id =:sme_id and ard.agent_id = ad.agent_id LIMIT :g_total_agent_count ) as total_count from agent_details ad left JOIN agent_details_timing adt ON ad.agent_id = adt.agent_id left JOIN agent_group_mapping agm ON ad.agent_id = agm.agent_id left JOIN agent_group_detail agd ON agm.group_id = agd.group_id  WHERE ad.sme_id =:sme_id  and ad.status = '1' and adt.days_week = UPPER(DATE_FORMAT(:currentDate, '%a'))  AND DATE_FORMAT(adt.in_time, '%T') < DATE_FORMAT(:currentDate, '%T') AND DATE_FORMAT(adt.out_time, '%T') > DATE_FORMAT(:currentDate, '%T') AND agd.group_name = (select agd2.group_name from agent_group_detail agd2 where agd2.group_id =:g_group) AND ad.agent_id NOT IN (:g_agent_ignore_list)  ORDER by total_count LIMIT 1 ";
      Query =
...
export async function getOffHoursAgent(where: any, callback: any) {
  try {
    ...
    //Query = "select count(*) as not_offhours_agents_count from agent_details_timing adt WHERE adt.sme_id =:sme_id  and adt.days_week = UPPER(DATE_FORMAT(:currentDate, '%a')) AND DATE_FORMAT(adt.in_time, '%T') < DATE_FORMAT(:currentDate, '%T')AND DATE_FORMAT(adt.out_time, '%T') > DATE_FORMAT(:currentDate, '%T')";
...
export async function getRandomAgentParallelRinging(where: any, callback: any) {
 ...
 console.log("query:",Query )
  ...
  let executeQuery = await sequelize_reader.query(Query, {
    raw: true,
    type: QueryTypes.SELECT,
    replacements: { sme_id: where["sme_id"], g_group: where["g_group"], currentDate: where["currentDate"], g_agent_ignore_list: where["g_agent_ignore_list"], g_total_agents: where["g_total_agents"] },
  });
  callback(null, executeQuery);
} catch (error: any) {
