            ...
            if(finalUploadCampaignData && finalUploadCampaignData.length > 0) {
              for (let l = 0; l < finalUploadCampaignData.length; l++) {
                ...
                let reqCustomerNumbers: any = {
                  ...
                  callStatus: 0,
                  sessionId: uniqueSessionId,
                  callMode: req.body.callMode ? req.body.callMode : 6
                };
...
export const updateIsPicked = async (req: Request, res: Response) => {
  try {

    let reqData: any = {
      smeId: parseInt(req.params.id),
      campaignId: req.body.campaign_id ? req.body.campaign_id : 0,
      id: req.body.id ? req.body.id : 1,
    };

    await updateIsPickedData(reqData, async(err: any, response: any) => {
      if (err) {
        return ErrorEmptyResponse(res, err);
      } else {
        return SuccessResponse(res, "Successfully listed", response);
