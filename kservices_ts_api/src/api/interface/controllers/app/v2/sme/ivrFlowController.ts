...
export const createivrflow = async (req: Request, res: Response) => {
  try {
    ...
    var assignedLongcodes = req.body.longcode;
    let where: any = {
      sme_id: req.params.id,
    };

    await getMaxFlowId(where, async(err: any, response: any) => {
      if (err) {
        return ErrorEmptyResponse(res, err);
      } else {
        var flowId = 0;
        ...
        for (let data of ivrData) {
          ...
          await insertUpdateIvrFlow(reqData, (err: any) => {
            if (err) {
              return ErrorEmptyResponse(res, err);
            } else {
        ...
        if(assignedLongcodes && assignedLongcodes.length > 0) {
          for (let assignedLongcode of assignedLongcodes) {
            let reqDataNew: any = {
              smeId: req.params.id,
              longcode: assignedLongcode["id"],
              flowId: flowId
            };
            await updateSmeLongcodeMapping(reqDataNew, (err: any) => {
              if (err) {
                return ErrorEmptyResponse(res, err);
              } else {
...
export const updateivrflow = async (req: Request, res: Response) => {
  try {
    ...
    var ivrData = req.body.list;
    var assignedLongcodes = req.body.longcode;
    await deleteIvrFlow(reqData, async(err: any, response: any) => {
      if (err) {
        return ErrorEmptyResponse(res, err);
      } else {
        await getMaxFlowId(reqData, async(err: any, response: any) => {
          if (err) {
            return ErrorEmptyResponse(res, err);
          } else {
            var flowId = 0;
            ...
            await resetSmeLongcodeFlowMapping(reqData, async(err: any) => {
              if (err) {
                return ErrorEmptyResponse(res, err);
              } else {
                ...
                if(assignedLongcodes && assignedLongcodes.length > 0) {
                  for (let assignedLongcode of assignedLongcodes) {
                    ...
                    await updateSmeLongcodeMapping(reqDataNew1, async(err: any) => {
                      if (err) {
                        return ErrorEmptyResponse(res, err);
                      } else {
  ...
  try {
    ...
    await updateQueueData(reqData, (err: any, response: any) => {
      if (err) {
        ...
        if(reqData["agentId"] && reqData["agentId"].length > 0 && req.body.mode == "update"){
          deleteQueueMappingData(reqData, async(err: any, response: any) => {
            if (err) {
              return ErrorEmptyResponse(res, err);
            } else {
              for(var i=0; i<reqData["agentId"].length; i++){
                ...
                await addQueueMappingData(reqData1, (err: any, response: any) => {
                  if (err) {
                    return ErrorEmptyResponse(res, err);
                  } else {
                  }
