...
import { FindCallList, setFollowUpCallData, followNumberExistInUniqueDetails, updateUniqueCustomerData, insertUniqueCustomerData, getMergeCallsData, clearLiveCallData, updateCallColumnSettings, getAgentLiveCallLeadData, getAgentLiveCallCampaignLeadData, getAgentMergeCallsData } from "../../../../../domain/models/v2/sme.model";
...
import { callListFetchRequest, mergeCallFetchRequest } from "../../../../../domain/entities/v2/sme.entity";
...
export const getMergeCalls = async (req: Request, res: Response) => {
  try {
    ...
    let reqData: any = {
      ...
      callDirection_op: callDirection_op,
      callStatus: callStatus,
      callStatus_op: callStatus_op,
      duration: duration,
      duration_op: duration_op,
      ...
      callQueue: callQueue,
      callQueue_op: callQueue_op,
      agentId: agentId,
      teamLead: teamLead ? teamLead : 0
    };
    if(agentId && agentId > 0 && teamLead == 0){
      await getAgentMergeCallsData(reqData, (err: any, response: any) => {
        if (err) {
          return ErrorEmptyResponse(res, err);
        } else {
          if(response.length > 0){
      ...
      await getMergeCallsData(reqData, (err: any, response: any) => {
        if (err) {
          return ErrorEmptyResponse(res, err);
        } else {
          if(response.length > 0){
